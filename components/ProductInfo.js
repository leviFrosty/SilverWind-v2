    ...
    <div className="flex flex-col max-w-xl">
      ...
      <div className="flex flex-col-reverse my-3">
        <Link href="/return-policy">
          <a className="text-violet-300 m-0 text-sm">*Shipping and return policies</a>
        </Link>
        <button onClick={validateCartAdd} className=" mx-2 cursor-pointer bg-violet-900 text-white rounded-md px-3 py-3 hover:bg-violet-500 font-bold focus:bg-violet-500 ">
          Add to cart
        </button>
      </div>
      ...
      <div className="product-description">
        <p className="text-violet-900 mx-6 my-4 leading-7">{product.description}</p>
      </div>
    </div>
  );
}
